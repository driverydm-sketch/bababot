...
// טעינת dotenv רק אם אנחנו בסביבת פיתוח מקומית (מונע קריסות בשרת ייצור)
if (process.env.NODE_ENV !== 'production') {
    try {
        require('dotenv').config();
    } catch (e) {
        console.log("⚠️ לא נמצא קובץ .env מקומי, משתמש במשתני המערכת.");
    }
}

// שליפת משתני הסביבה מהשרת (Render / Local)
...
// בדיקה חיונית ללא קריסת שרת - הדפסת שגיאה ברורה ללוגים
if (!supabaseUrl || !supabaseKey || !botToken) {
    console.error("❌ אזהרה: חסרים חלק ממשתני הסביבה (SUPABASE_URL, SUPABASE_KEY או TELEGRAM_TOKEN)!");
}

// אתחול הקליינטים (רק אם המשתנים קיימים, למניעת קריסות איתחול)
...
if (bot) {
    ...
    bot.command('bet', async (ctx) => {
        if (!supabase) return ctx.reply("❌ חיבור לבסיס הנתונים לא זמין.");
        try {
    ...
    bot.on('callback_query', async (ctx) => {
        if (!supabase) return ctx.answerCbQuery("❌ בסיס הנתונים מנותק.", { show_alert: true });
        try {
            const data = ctx.data;
            const userId = ctx.from.id;
